...
public class Routes {
    ...
    private static final Map<Route, RouteData> routes = Map.ofEntries(
            ...
            new AbstractMap.SimpleEntry<>(Route.BUILDINGS_DETAILS_RAPORT, new RouteData().title("Generowanie raportu")),
            new AbstractMap.SimpleEntry<>(Route.WORK_RAPORT_STATUS, new RouteData().title("Generowanie raportu"))
    );
